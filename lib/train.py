...
def step(split, epoch, opt, dataLoader, model, criterion, optimizer = None):
  ...
  for i, (input, target, mask) in enumerate(dataLoader):
    ...
    if opt.GPU > -1:
      ...
      target_var = torch.autograd.Variable(target.cuda(opt.GPU, non_blocking = True)).float().cuda(opt.GPU)
      mask_var = torch.autograd.Variable(mask.cuda(opt.GPU, non_blocking = True)).float().cuda(opt.GPU)
    else:
    ...
    if opt.DEBUG > 1 or (opt.DEBUG == 1 and i % (nIters / 200) == 0):
      for j in range(input.size(0)):
        ...
        imgMNS = img.copy()
        out = (cv2.resize(((output[opt.nStack - 1][j, 0].data).cpu().numpy()).copy(), (opt.inputRes, opt.inputRes)) * 256)
        gtmap = (cv2.resize((target[j, 0].cpu().numpy()).copy(), (opt.inputRes, opt.inputRes)) * 256)
        out[out < 0] = 0
        out[out > 255] = 255
        img2[:,:,0] = (img2[:,:,0] + out)
        img2[img2 > 255] = 255
