...
filter_foo = filter_func(settings)
# occ_level = 'FGL3_BGL3'
occ_level = ''

if len(occ_level) == 0:
    IMAGE_PATH = PATH + 'Images/%s/'
else:
    IMAGE_PATH = '/ccvl/net/ccvl15/angtian/workspace/PASCAL3D+/PASCAL_NEWs/images/%s%s/' % ('%s', occ_level)
        ...
        for (t, im_name) in enumerate(os.listdir(img_path)):
            ...
            for i in range(data['anchors'].shape[0]):
                ...
                if len(anchors) > 0 and len(viewpoint) > 0:
                    ...
                    if not filter_foo(data['viewpoint'][i][0]['azimuth'][0][0][0] * np.pi / 180):
                        continue
                    cnt += 1
