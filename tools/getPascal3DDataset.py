...
if len(occ_level) == 0:
    ...
    IMAGE_PATH = '/home/zhangg/StarMap/data/OccludedPASCAL3D/images/%s%s/' % ('%s', occ_level)

IMAGE_PATH = '/home/zhangg/Original_NeMo/data/OOD_test_NeMo/iid_test/images/%s/'
...
# objs = ['train_imagenet', 'bus_pascal', 'aeroplane_pascal', 'diningtable_imagenet', 'chair_imagenet', 'boat_imagenet', 'bicycle_imagenet', 'bicycle_pascal',  'bus_imagenet', 'aeroplane_imagenet', 'chair_pascal', 'diningtable_pascal',  'train_pascal', 'boat_pascal', 'motorbike_imagenet','motorbike_pascal', 'car_imagenet', 'car_pascal', 'sofa_imagenet', 'sofa_pascal']
objs = ['train_imagenet',  'diningtable_imagenet', 'chair_imagenet', 'boat_imagenet', 'bicycle_imagenet', 'bus_imagenet', 'aeroplane_imagenet',  'motorbike_imagenet', 'car_imagenet', 'sofa_imagenet']

objs = [t for t in objs if 'imagenet' in t]
...
if split == 'val':
    OOD_data = 'weather'
    IMAGE_PATH = '/home/zhangg/Original_NeMo/data/OOD_test_NeMo/nuisances/weather/images/'
    ANNOT_PATH = '/home/zhangg/Original_NeMo/data/OOD_test_NeMo/nuisances/weather/annotations/'
    LIST_PATH = '/home/zhangg/NeMo/data/OOD_test_NeMo/nuisances/weather/lists3D_single/'
    objs = ['aeroplane', 'bicycle', 'boat', 'bus', 'car', 'chair', 'diningtable', 'motorbike', 'sofa', 'train']
    annot = {}
    tags = ['bbox', 'cad_index']
    tags_viewpoint = ['azimuth', 'elevation', 'distance', 'focal', 'theta', 'viewport']
    keys = tags + ['viewpoint_' + v for v in tags_viewpoint] + ['class_id', 'imgname', 'principal']
    for tag in keys:
        annot[tag] = []
    for cate in objs:
        cnt = 0
        image_path = os.path.join(IMAGE_PATH, cate)
        anno_path = os.path.join(ANNOT_PATH, cate)
        f_path = os.path.join(LIST_PATH, cate, 'mesh01.txt')
        f = open(f_path, 'r')
        for fname in f.readlines():
            fname = fname.strip('\n')
            # print(fname)
            f_anno_path = os.path.join(anno_path, fname[:fname.find('.')] + '.npz')
            # print(f_anno_path)
            if os.path.exists(f_anno_path):
                cnt += 1
                anno = np.load(f_anno_path, allow_pickle=True)
                for key in anno.keys():
                    if key in tags_viewpoint:
                        annot['viewpoint_' + key] += [anno[key]]
                        continue
                    elif key in tags or key == 'principal':
                        annot[key] += [anno[key]]
                        continue
                    elif key == 'name':
                        imgname_ = np.zeros(100, dtype = np.uint8)
                        im_name = fname
                        # print(im_name)
                        for v in range(len(im_name)): 
                            imgname_[v] = ord(im_name[v])
                        annot['imgname'] += [imgname_]
                        # annot['imgname'] += [anno[key]]
                        continue
                    else:
                        continue
                annot['class_id'] += [objs.index(cate)]
        print(cate + ': ' + str(cnt))
    import os
    path = '../data/OOD_test/'
    if not os.path.exists(path):
        os.makedirs(path)
    with h5py.File(path + 'Pascal3D_{}-{}.h5'.format(OOD_data, split),'w') as f:
        f.attrs['name'] = 'Pascal3D_{}-{}'.format(OOD_data, split)
        for k in keys:
...
if split == ['train']:
    ...
    total = 0
    annot = {}
    keys = tags + ['viewpoint_' + v for v in tags_viewpoint] + ['status', 'vis', 'dataset', 'class_id', 'imgname', 'projected_anchors', 'anchors_3d', 'valid', 'space_embedding', 'avg_embedding', 'principal']
    for tag in keys:
        annot[tag] = []
    maxImgName = -1
    ...
    for obj in objs:
        ...
        if '%s' in IMAGE_PATH:
            img_path = IMAGE_PATH % obj[:obj.find('_')]
        else:
        ...
        for (t, im_name) in enumerate(os.listdir(img_path)):
            ...
            # print(im_name[0])
            if im_name[0] != 'n':
                obj = obj[:obj.find('_')] + '_' + 'pascal'
            else:
                obj = obj[:obj.find('_')] + '_' + 'imagenet'
                    ...
                    if len(data['cad_index'][i][0]) > 0:
                        try:
                            ...
                            if DEBUG:
                                ...
                                img_path = '{}/{}'.format(IMAGE_PATH % obj[:obj.find('_')], im_name)
                                print(img_path)
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    with h5py.File(path + 'Pascal3D_{}-{}.h5'.format(occ_level, split),'w') as f:
        f.attrs['name'] = 'Pascal3D_{}-{}'.format(occ_level, split)
        for k in keys:
            f[k] = np.array(annot[k])
